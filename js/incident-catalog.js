...
const INCIDENT_DEFINITIONS={
  ...
  airport_capacity_reduction:{title:'Airport flow restriction',severity:'warning',decisionMin:35,summary:'A temporary airport, surface, or ATC flow restriction is affecting departure flow.'},
...
const INCIDENT_DEFAULT_POLICIES={
  ...
  gate_conflict:{mode:'accept_delay',label:'accept delay',strategy:'wait_gate',delayMin:45,summary:'If Station Operations takes no action before the decision deadline, the flight waits for the planned gate and absorbs the stand delay.'},
  ...
  destination_closure:{mode:'flightdeck_default',label:'flight deck plan',summary:'If OCC does not complete the case before the deadline, the flight deck makes the operational decision using the available fuel, weather, and alternate picture; OCC records the resulting support plan.'},
  ...
  baggage_loading_issue:{mode:'accept_delay',label:'accept delay',strategy:'baggage_reload',delayMin:35,summary:'If Station Operations takes no action before the decision deadline, load-control closeout waits for station reconciliation and the delay is accepted.'},
  fueling_issue:{mode:'accept_delay',label:'accept delay',strategy:'wait_truck',delayMin:35,summary:'If Station Operations takes no action before the decision deadline, the fuel-truck queue is accepted and the departure is retimed.'},
  ...
  airport_capacity_reduction:{mode:'accept_delay',label:'accept delay',strategy:'accept',summary:'If Dispatch takes no action before the decision deadline, the assigned airport-flow sequence is accepted and the delay is published.'},
  ...
  bird_strike:{mode:'flightdeck_default',label:'flight deck plan',summary:'If OCC does not complete the case before the deadline, the flight deck decides continue, divert, or return using aircraft status; OCC records the support plan.'},
  onboard_medical:{mode:'flightdeck_default',label:'medical plan',summary:'If OCC does not complete the case before the deadline, medical advisory and the captain determine continue or divert; OCC records arrival or diversion support.'},
  inflight_technical_fault:{mode:'flightdeck_default',label:'flight deck plan',summary:'If OCC does not complete the case before the deadline, the flight deck decides continue or divert after maintenance-control guidance; OCC records the support plan.'},
  fuel_margin_low:{mode:'flightdeck_default',label:'fuel plan',summary:'If OCC does not complete the case before the deadline, the flight deck uses the available fuel picture to choose conservation, priority routing, diversion, or return; OCC records the support plan.'},
  atc_holding_fuel_conflict:{mode:'flightdeck_default',label:'fuel plan',summary:'If OCC does not complete the case before the deadline, the flight deck chooses priority routing or diversion using the current fuel picture; OCC records the support plan.'},
  ...
  unruly_passenger:{mode:'flightdeck_default',label:'security plan',summary:'If OCC does not complete the case before the deadline, the captain decides continue or divert; OCC records destination or diversion security support.'},
  destination_weather_deterioration:{mode:'flightdeck_default',label:'weather plan',summary:'If OCC does not complete the case before the deadline, the flight deck decides monitor, hold, or divert using the latest destination weather picture; OCC records the support plan.'},
  destination_below_minima:{mode:'flightdeck_default',label:'weather plan',summary:'If OCC does not complete the case before the deadline, the flight deck decides hold, divert, or return using fuel, minima, and alternate information; OCC records the support plan.'},
  diversion_airport_unavailable:{mode:'flightdeck_default',label:'diversion plan',summary:'If OCC does not complete the case before the deadline, the flight deck decides hold, reselect, or return based on the updated diversion-airport picture; OCC records the support plan.'},
  lightning_strike:{mode:'flightdeck_default',label:'flight deck plan',summary:'If OCC does not complete the case before the deadline, the flight deck decides continue or divert after systems checks; OCC records inspection support.'},
  pressurization_issue:{mode:'flightdeck_default',label:'flight deck plan',summary:'If OCC does not complete the case before the deadline, the flight deck decides lower-altitude continuation or diversion; OCC records fuel and arrival support.'}
};
