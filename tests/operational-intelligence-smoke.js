...
const onward=[{id:'AS2',from:'LHR',to:'JFK',departure:now+2*HOUR+50*MIN}];
const connections=AeroOperationalIntelligence.connectionManifest({flight,onwardFlights:onward,actualArrival:now+2*HOUR+20*MIN});
assert.ok(connections.total>0);
assert.equal(connections.missed,connections.total);
